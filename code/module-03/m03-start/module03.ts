...
function calculateConventionalLoanPayment(conventionalLoanTerms: ConventionalLoan): string {
    ...
    let payment:number = conventionalLoanTerms.principal * interest / (1 - (Math.pow(1 / (1 + interest), conventionalLoanTerms.months)));
...
let conventionalLoanTerms: ConventionalLoan = {
    principal: 3000,
    interestRate: 5,
    months: 180
}
